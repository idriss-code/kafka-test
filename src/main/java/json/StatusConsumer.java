...
public class StatusConsumer {
    public static void main(String[] args) {
        ...
        consumerProperties.put(JsonDeserializer.CONFIG_VALUE_CLASS, TaskStatus.class.getName());

        KafkaConsumer<String, TaskStatus> consumer = new KafkaConsumer<>(consumerProperties);

        consumer.subscribe(Arrays.asList("json-status"));

        while (true) {
            ConsumerRecords<String, TaskStatus> records = consumer.poll(Duration.ofSeconds(10));
